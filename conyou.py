...
# =========================================================================
# INJECTION DU STYLE CSS GLOBAL DE L'APPLICATION
# =========================================================================
st.markdown("""
<style>
...
# =========================================================================
# MENUS DE NAVIGATION SUR LA BARRE LATÉRALE
# =========================================================================
with st.sidebar:
...
# =========================================================================
# REDIRECTION ET CHARGEMENT DYNAMIQUE DES MODULES
# =========================================================================
try:
    ...
    st.error(f"⚠️ Erreur de chargement du module : **{e.name}**.")
    st.info("Veuillez vérifier que le dossier `modules/` existe bien et contient tous les fichiers nécessaires.")
    import streamlit as st
import pandas as pd
import json
import datetime
...
def show():
    """
    Module complet : Assistant IA de Décision Stratégique & Agribusiness Sénégal
    """
    
    # Styles CSS dédiés à l'interface d'Analyse IA
    st.markdown("""
    <style>
    .ai-hero {
        padding: 30px 25px;
    ...
    # =========================================================================
    # FORMULAIRE DE CADRAGE DU PROJET
    # =========================================================================
    ...
    # =========================================================================
    # MOTEUR DE GÉNÉRATION DYNAMIQUE DU DIAGNOSTIC
    # =========================================================================
    if btn_generer:
        if not description_projet or len(description_projet.strip()) < 20:
            ...
            with st.spinner("🧠 Analyse contextuelle en cours... Évaluation des facteurs réglementaires, financiers et agronomiques..."):
                ...
                # Calculs et simulations basés sur les entrées
                score_base = 70
                ...
                diag = {
                    ...
                    "risques": [
                        {"Risque": "Retard de livraison des équipements d'irrigation/transformation", "Niveau": "Moyen", "Impact Financial": "Modéré", "Plan d'Atténuation": "Passer commande via des fournisseurs agréés SAED/ANCAR avec clauses pénales."},
                        {"Risque": "Volatilité des cours locaux au moment de la récolte", "Niveau": "Élevé", "Impact Financial": "Important", "Plan d me Atténuation": "Négocier des contrats d'achat ferme à prix plancher avant le démarrage de la campagne."},
                        {"Risque": "Défaillance de la maintenance du matériel solaire/mécanique", "Niveau": "Moyen", "Impact Financial": "Élevé", "Plan d'Atténuation": "Souscrire un contrat d'entretien préventif avec garantie de pièces de rechange sous 48h."}
                    ],
                    "plan_action": [
    ...
    if st.session_state.dernier_diagnostic:
        ...
        # Tabs de présentation approfondie
        ...
        # TAB 4 : PLAN D'ACTION
        with tab_plan:
            st.markdown("#### Feud de Route Opérationnelle (Chrono-programme)")
...
"""
        ...
        if st.button("💬 Poser la question"):
            if question.strip():
                ...
                # Moteur de réponse contextuelle
                ...
                if "der" in q_low or "finance" in q_low or "credit" in q_low:
                    ...
                    rep = "L'autorisation FRA (Fabrication et Repconditionnement Alimentaire) s'obtient auprès de la Division de la Consommation. Le dossier comprend un plan des locaux, une analyse micro-biologique produit et la visite des inspecteurs."
                else:
...
def show():
    """
    Module complet : Tableau de Bord Analytique & Suivi des Marchés Sénégal
    """
    ...
    .dash-hero {
        ...
        box-shadow: 0 4px 15px rgba(5, 46, 22, 0.15);
        border-bottom: 4px solid #e1a91a;
        margin-bottom: 25px;
    }
    .dash-hero h2 {
    ...
    # Alerte météo/parasitaire en cours
    st.markdown("""
    <div class="alert-box">
    ...
    # =========================================================================
    # GRAPHIQUES DE PRODUCTION ET PRÉVISIONS
    # =========================================================================
    ...
    with col_g1:
        ...
        # Données simulées d'évolution mensuelle
    ...
    # =========================================================================
    # SUIVI DES COURS DES MARCHÉS LOCAUX (Bana-Bana & Grossistes)
    # =========================================================================
    ...
    with m_col1:
        ...
        <div class="market-card">
            <div class="market-title">🌾 Riz Blanc LocalDécortiqué</div>
    ...
    df_m = pd.DataFrame(data_marches)
    st.dataframe(df_m, use_container_width=True, hide_index=True)

    # =========================================================================
    # ANACIM & METEO AGRONOMIQUE
    # =========================================================================
    st.markdown("---")
...
def show():
    """
    Module complet : Gestion des Cultures, Irrigation & Calendriers ISRA / DPV
    """
    ...
    # =========================================================================
    # TAB 2 : CALCULATEUR D'IRRIGATION
    # =========================================================================
    with tab_irrigation:
    ...
    with tab_intrants:
        ...
        if crop_spec == "Riz de Bas-fond / Irrigué":
            fert_data = [
                ...
                {"Stade d me Application": "Début Tallage (15-20 jours)", "Type d'Engrais": "Urée (46%)", "Dose Recommandée": "100 Kg / Ha", "Mode d'Apport": "Épandage à la volée sur sol humide"},
            ...
            fert_data = [
                ...
                {"Stade d'Application": "3 Semaines après repiquage", "Type d'Engrais": "Nitrate de Calcium / Urée", "Dose Recommandée": "75 Kg / Ha", "Mode d me Apport": "Irrigation ou fertigation"},
    ...
    # =========================================================================
    # TAB 4 : PROTECTION PHYTOSANITAIRE (DPV)
    # =========================================================================
    with tab_sante:
...
def show():
    """
    Module complet : Financement, Subventions & Crédits Agricoles Sénégal
    """
    ...
    .finance-hero {
        ...
        border-radius: 12px;
        color: white;
        background: linear-gradient(135deg, #854d0e 0%, #361e04 100%);
        box-shadow: 0 4px 15px rgba(54, 30, 4, 0.2);
        border-bottom: 4px solid #e1a91a;
        margin-bottom: 25px;
    }
    .finance-hero h2 {
    ...
    # =========================================================================
    # TAB 1 : SIMULATEUR DE CRÉDIT ET ÉCHÉANCIER
    # =========================================================================
    with tab_simu:
        ...
        with c_f2:
            ...
            # Détermination du taux selon l'institution
            ...
            # Calcul mensuel (Formule amortissement constant / dégressif)
            ...
            # Vérification éligibilité apport
            if ratio_apport >= 10:
            ...
            if st.checkbox("📊 Afficher le Tableau d'Amortissement Prévionnel"):
                schedule = []
                solde = emprunt
    ...
    # =========================================================================
    # TAB 2 : DISPOSITIFS DE FINANCEMENT SÉNÉGAL
    # =========================================================================
    with tab_guichets:
    ...
    # =========================================================================
    # TAB 3 : SUBVENTIONS ET ÉQUIPEMENTS
    # =========================================================================
    with tab_subventions:
        ...
        st.table(pd.DataFrame(data_subventions))import streamlit as st

def show():
    """
    Module complet : Page d'Accueil Interactive
    """
    ...
    # Aperçu rapide / Métriques d'impact
...
def show():
    """
    Module complet : Support Technique, Assistance & Contact
    """
