...
# IMPORTANT : ne jamais modifier la clé d'un widget après son instanciation.
# Les boutons déposent une cible, puis elle est appliquée avant la création du widget.
...
if space == "🏠 Accueil":
    ...
    if a.button("📁 Ouvrir le dossier",use_container_width=True,key="quick_dossier"):
        st.session_state["v10_space_target"]="🌍 Terrain"; st.session_state["terrain_v10_target"]="📁 Dossier 360°"; st.rerun()
    if b.button("💬 Entretien agriculteur",use_container_width=True,key="quick_interview"):
        st.session_state["v10_space_target"]="🌍 Terrain"; st.session_state["terrain_v10_target"]="💬 Entretien / Messages"; st.rerun()
    if c.button("🧠 Diagnostic 360°",use_container_width=True,key="quick_diag"):
        st.session_state["v10_space_target"]="🗺️ Diagnostic"; st.session_state["diagnostic_v10_target"]="🔬 Diagnostic 360°"; st.rerun()
    if d.button("📄 Rapports",use_container_width=True,key="quick_report"):
        st.session_state["v10_space_target"]="💼 Cabinet"; st.session_state["cabinet_v10_target"]="📄 Rapports"; st.rerun()

elif space == "🌍 Terrain":
    st.markdown("### 🌍 Terrain")
    terrain_options=["📁 Dossier 360°","🌾 Production","👁️ Observations","🧪 Analyses",
        "💬 Entretien / Messages","📨 Communications","📦 Équipements","📚 Historique"]
    _target=st.session_state.pop("terrain_v10_target", None)
    _idx=terrain_options.index(_target) if _target in terrain_options else 0
    section=st.selectbox("Rubrique",terrain_options,index=_idx,key="terrain_v10_section")
    mapping={
    ...
    diagnostic_options=["🗺️ Zone & GPS","🧭 Couches SIG","🌱 Sols & Eau","🦠 Santé / Phytosanitaire",
        "🌦️ Climat & risques","🔎 Qualité & preuves","🔬 Diagnostic 360°"]
    _target=st.session_state.pop("diagnostic_v10_target", None)
    _idx=diagnostic_options.index(_target) if _target in diagnostic_options else 0
    section=st.selectbox("Rubrique",diagnostic_options,index=_idx,key="diagnostic_v10_section")
    mapping={
    ...
    st.markdown("### 💼 Cabinet")
    cabinet_options=["👥 Clients & dossiers","📋 Missions","💰 Devis & Finance","📄 Rapports",
        "📚 Documents","📆 Suivi & agenda","⚙️ Administration","🔐 Accès utilisateurs","🛡️ Traçabilité"]
    _target=st.session_state.pop("cabinet_v10_target", None)
    _idx=cabinet_options.index(_target) if _target in cabinet_options else 0
    section=st.selectbox("Rubrique",cabinet_options,index=_idx,key="cabinet_v10_section")
    mapping={
